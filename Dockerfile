...
RUN install -d -o www-data -g www-data /var/lib/fenping/netboot \
    ...
    && printf '%s\n' \
      ...
      'permit nopass setenv { DATABASE_PATH DHCP_NETWORK EXTRA_NETWORKS SCAN_NETWORK INVENTORY_DOWN_RETENTION_DAYS IFACE IP PASSWORD SECRET DISCORD_WEBHOOK_URL DISCORD_MENTION TELEGRAM_BOT_TOKEN FENPING_DATA_DIR DNSMASQ_RELOAD_MODE } www-data as root cmd /usr/bin/php args /opt/fenping/cli.php inventory --work' \
