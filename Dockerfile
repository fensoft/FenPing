...
RUN apt-get update && apt-get install -y cron nano apache2 mariadb-server libapache2-mod-php php-mysql isc-dhcp-server bind9 libxml-xpath-perl nmap iputils-ping php-curl sudo iptables composer curl unzip syslog-ng && apt clean; rm /var/www/html/index.html
