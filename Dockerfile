...
RUN apt-get update && apt-get install -y cron nano apache2 mariadb-server libapache2-mod-php php-mysql isc-dhcp-server bind9 libxml-xpath-perl nmap iputils-ping php-curl sudo iptables && apt clean; rm /var/www/html/index.html
...
RUN echo 'include "/etc/dhcp/dhcpd.hosts";' >> /etc/dhcp/dhcpd.conf
