...
function pingHosts($ips, $interface, $localIps) {
  $hosts = array();
...
function pingHost($ip, $interface, $localIps = array(), $localMac = null, $arpTable = null, $icmpUp = null, $db = null) {
  $mac = "";

  if (in_array($ip, $localIps, true)) {
    $host = array("ip" => $ip, "mac" => $localMac === null ? localMac($interface) : $localMac, "status" => "Up");
    savePingHost($host, $db);
    return $host;
  }

  if ($icmpUp === null)
    $icmpUp = pingSweep(array($ip), $interface, array());
  if ($arpTable === null)
    $arpTable = readArpTable($interface);
  ...
  if ($status === "Down" && $mac !== "") {
    $status = "arp";
    if (!arpingReplies($ip, $interface)) {
      $status = "arp-down";
    } elseif (singlePing($ip, $interface)) {
      $status = "Up";
    }
  }
...
function readArpTable($preferredInterface) {
...
function pingSweep($ips, $interface, $skipIps) {
  $result = array();
  ...
  if ($socket === false) {
    foreach ($ips as $ip) {
      if (!isset($result[$ip]))
        $result[$ip] = systemPing($ip, $interface);
    }
    return $result;
  }
...
function singlePing($ip, $interface) {
  $result = pingSweep(array($ip), $interface, array());
  return !empty($result[$ip]);
}

function createIcmpSocket() {
...
function icmpPacket($id, $sequence) {
...
function parseIcmpReply($buffer) {
  if (strlen($buffer) < 28)
    return null;
...
function arpingReplies($ip, $interface) {
  if ($interface === "")
    return false;
...
function systemPing($ip, $interface) {
  $cmd = "ping -c 1 -W 1 ";
  if ($interface !== "")
