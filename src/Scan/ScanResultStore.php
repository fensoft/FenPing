...
final readonly class ScanResultStore
{
...
public function scanMetadataBestResult(string $ip, ?string $mode = null): ?array {
  if ($mode !== null && !$this->profiles->isValid($mode))
...
public function scanMergePorts(array $deep, array $partial, string $partialSource): array {
  ...
  foreach ($this->scanMarkResultSource($partial, $partialSource) as $item) {
    $key = ($item['protocol'] ?? '') . '|' . ($item['port'] ?? '');
    $items[$key] = $this->portKnowledge->merge($items[$key] ?? null, $item);
  }
  return array_values($items);
}
...
public function scanMetadataHistory(string $ip, int $limit = 30): array {
  ...
  while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
    ...
    if ($this->metadataXmlUsable($metadata))
      $history[] = $metadata;
  }
  return $history;
}
...
public function scanMetadataForIp(string $ip, int $limit = 50): array {
  ...
  while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
    $metadata = $this->policy->normalizeMetadata($row);
    $metadata['xml_usable'] = $this->metadataXmlUsable($metadata);
    $metadata['xml_url'] = $metadata['xml_usable'] ? $this->codec->url($metadata['ip'], $metadata['id']) : null;
    $scans[] = $metadata;
  }
  return $scans;
}
...
public function scanMetadataQueue(int $limit = 100): array {
  ...
  $queue = array();
  $policyState = $this->policy->scanQueuePolicyState();
  while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
    $metadata = $this->policy->normalizeMetadata($row, $policyState['annotations']);
...
public function scanMetadataLatestByIp(): array {
  ...
  $scans = array();
  while ($row = $stmt->fetch(PDO::FETCH_ASSOC))
    $scans[$row['ip']] = $this->policy->normalizeMetadata($row);
  return $scans;
}
...
private function metadataXmlUsable(array $metadata): bool {
