...
public function create_netboot_image(array $file, string $name = "") {
  ...
  return $this->get_netboot_image((int)$this->getDb()->lastInsertId());
}
...
public function delete_netboot_image(int $id): array {
  ...
  $stmt = $this->getDb()->prepare("UPDATE ips SET netboot_image_id=NULL WHERE netboot_image_id=:id");
  $stmt->execute(array("id" => $id));

  $stmt = $this->getDb()->prepare("DELETE FROM netboot_images WHERE id=:id");
  $stmt->execute(array("id" => $id));
...
public function get_netboot_image(int $id) {
  $stmt = $this->getDb()->prepare("SELECT id, name, filename, original_name, size, created_at FROM netboot_images WHERE id=:id");
  $stmt->execute(array("id" => $id));
