<?php

declare(strict_types=1);

namespace FenPing\Backend;

use InvalidArgumentException;
use RuntimeException;
use Throwable;

trait ApiBehavior
{
public function dispatchRequest(): void {
  $method = $_SERVER['REQUEST_METHOD'] ?? 'GET';
  $segments = $this->routeSegments();

  if (count($segments) === 0)
    $this->jsonError(404, 'not found');

  $methodAllowed = false;
  $invalidError = null;

  foreach ($this->apiRoutes() as $route) {
    $match = $this->matchRoutePath($route['pattern'], $segments);

    if (!$match['matched']) {
      if ($match['invalid'] && $route['method'] === $method && $invalidError === null)
        $invalidError = $match['error'];
      continue;
    }

    if ($route['method'] !== $method) {
      $methodAllowed = true;
      continue;
    }

    $this->authorizeRoute($route);
    $result = call_user_func($route['handler'], $match['params']);
    $this->jsonResponse($result);
  }

  if ($methodAllowed)
    $this->jsonError(405, 'method not allowed');

  if ($invalidError !== null)
    $this->jsonError(400, $invalidError);

  $this->jsonError(404, 'not found');
}

public function apiRoutes(): array {
  return array_merge(
    $this->authApiRoutes(),
    $this->systemApiRoutes(),
    $this->hostsApiRoutes(),
    $this->ipamApiRoutes(),
    $this->netbootApiRoutes(),
    $this->scansApiRoutes()
  );
}

public function apiRoute(string $method, string $pattern, string $handler, $auth = false, array $options = array()): array {
  return array_merge(array(
    'method' => strtoupper($method),
    'pattern' => $pattern,
    'handler' => [$this, $handler],
    'auth' => $auth
  ), $options);
}

public function routeSegments(): array {
  $path = parse_url($_SERVER['REQUEST_URI'] ?? '/', PHP_URL_PATH);
  $path = rawurldecode($path === false ? '/' : $path);

  if (str_starts_with($path, '/api.php')) {
    $path = substr($path, strlen('/api.php'));
  } elseif (str_starts_with($path, '/api')) {
    $path = substr($path, strlen('/api'));
  }

  $path = trim($path, '/');
  if ($path === '')
    return array();

  return array_values(array_filter(explode('/', $path), fn($part) => $part !== ''));
}

public function matchRoutePath(string $pattern, array $segments): array {
  $pattern = trim($pattern, '/');
  $parts = $pattern === '' ? array() : explode('/', $pattern);

  if (count($parts) !== count($segments))
    return $this->routeNoMatch();

  $params = array();
  foreach ($parts as $index => $part) {
    $segment = $segments[$index];
    if (preg_match('/^\{([A-Za-z_][A-Za-z0-9_]*)(?::([A-Za-z][A-Za-z0-9_]*))?\}$/', $part, $matches)) {
      $type = $matches[2] ?? 'string';
      $converted = $this->convertRouteParam($type, $segment);
      if (!$converted['ok'])
        return $this->routeInvalid($converted['error']);
      $params[$matches[1]] = $converted['value'];
      continue;
    }

    if ($part !== $segment)
      return $this->routeNoMatch();
  }

  return array('matched' => true, 'invalid' => false, 'params' => $params, 'error' => null);
}

public function routeNoMatch(): array {
  return array('matched' => false, 'invalid' => false, 'params' => array(), 'error' => null);
}

public function routeInvalid(string $error): array {
  return array('matched' => false, 'invalid' => true, 'params' => array(), 'error' => $error);
}

public function convertRouteParam(string $type, string $value): array {
  if ($type === 'int') {
    if (!ctype_digit($value))
      return $this->routeParamError('invalid id');
    return $this->routeParamValue((int)$value);
  }

  if ($type === 'ipv4') {
    if (filter_var($value, FILTER_VALIDATE_IP, FILTER_FLAG_IPV4) === false)
      return $this->routeParamError('invalid ip');
    return $this->routeParamValue($value);
  }

  if ($type === 'scanXml') {
    if (!preg_match('/^(\d{1,3}(?:\.\d{1,3}){3})\.xml$/', $value, $matches))
      return $this->routeParamError('invalid scan file');
    if (filter_var($matches[1], FILTER_VALIDATE_IP, FILTER_FLAG_IPV4) === false)
      return $this->routeParamError('invalid ip');
    return $this->routeParamValue($value);
  }

  if ($type === 'scanIdXml') {
    if (!preg_match('/^\d+\.xml$/', $value))
      return $this->routeParamError('invalid scan id');
    return $this->routeParamValue($value);
  }

  if ($value === '')
    return $this->routeParamError('invalid value');

  return $this->routeParamValue($value);
}

public function routeParamValue($value): array {
  return array('ok' => true, 'value' => $value, 'error' => null);
}

public function routeParamError(string $error): array {
  return array('ok' => false, 'value' => null, 'error' => $error);
}

public function authorizeRoute(array $route): void {
  $auth = $route['auth'] ?? false;
  if ($auth === false)
    return;

  if ($auth === 'body') {
    $this->requireAuth($this->requestBody());
    return;
  }

  $this->requireAuth();
}

public function requestBody(): array {
  $body = \FenPing\Api\RequestContext::body();
  return $body ?? $_POST;
}

public function requireAuth(?array $body = null): void {
  if ($this->authIsAuthenticated())
    return;

  if ($body !== null && array_key_exists('password', $body) && $this->authLogin($body['password']))
    return;

  $this->jsonError(403, 'login required');
}

public function normalizeHostIp($ip): ?string {
  $ip = trim((string)$ip);
  if ($ip === '')
    return null;
  if (strpos($ip, '.') === false)
    $ip = ($this->config->network) . '.' . $ip;
  if (filter_var($ip, FILTER_VALIDATE_IP, FILTER_FLAG_IPV4) === false)
    $this->jsonError(400, 'invalid ip');
  return $ip;
}

public function normalizeEmpty($value): ?string {
  $value = trim((string)$value);
  return $value === '' ? null : $value;
}

public function toDbFlag($value): ?string {
  if ($value === true || $value === 1 || $value === '1')
    return '1';
  return null;
}

public function normalizeNetbootImageId($value): ?int {
  if ($value === null || $value === '' || $value === false)
    return null;

  if (is_int($value))
    $id = $value;
  else {
    $value = trim((string)$value);
    if (!ctype_digit($value))
      $this->jsonError(400, 'invalid netboot image');
    $id = (int)$value;
  }

  if ($id <= 0 || !$this->netboot_image_exists($id))
    $this->jsonError(400, 'invalid netboot image');

  return $id;
}

public function reloadDhcpHosts(): string {
  try {
    return $this->runDhcpHostsCli('');
  } catch (RuntimeException $e) {
    $this->jsonError(500, $e->getMessage());
  }
}

public function commitDhcpMutation(callable $mutation): array {
  $lock = $this->acquireDnsmasqUpdateLock();
  $database = $this->db();
  $candidateDir = $this->dnsmasqPendingDir();
  $applied = false;
  $configuring = false;

  try {
    if ($database->inTransaction())
      throw new RuntimeException('database transaction already active');

    $this->clearDnsmasqCandidateDir($candidateDir);
    $this->dbBeginImmediate($database);
    $result = $mutation();
    $configuring = true;
    $this->prepareDnsmasqCandidate($this->buildDnsmasqFiles(), $candidateDir);
    $log = $this->runDhcpHostsCli('--apply-pending');
    $applied = true;
    $this->dbCommit($database);
    $this->operations->started('dnsmasq_generation');
    $this->operations->succeeded('dnsmasq_generation');

    return array('result' => $result, 'log' => $log);
  } catch (Throwable $error) {
    $recoveryErrors = array();

    if ($database->inTransaction()) {
      try {
        $this->dbRollback($database);
      } catch (Throwable $rollbackError) {
        $recoveryErrors[] = 'database rollback failed: ' . $rollbackError->getMessage();
      }
    }

    if ($applied) {
      try {
        $this->runDhcpHostsCli('--sync-locked');
      } catch (Throwable $syncError) {
        $recoveryErrors[] = 'dnsmasq recovery failed: ' . $syncError->getMessage();
      }
    }

    if ($configuring) {
      $this->operations->started('dnsmasq_generation');
      $this->operations->failed('dnsmasq_generation', $error->getMessage());
    }
    if (count($recoveryErrors) !== 0) {
      throw new RuntimeException(
        'DHCP update failed: ' . $error->getMessage() . '; ' . implode('; ', $recoveryErrors),
        0,
        $error
      );
    }
    throw $error;
  } finally {
    $this->clearDnsmasqCandidateDir($candidateDir);
    $this->releaseDnsmasqUpdateLock($lock);
  }
}

public function runDhcpHostsCli(string $mode): string {
  if (!in_array($mode, array('', '--apply-pending', '--sync-locked'), true))
    throw new InvalidArgumentException('invalid DHCP apply mode');

  $command = '/usr/bin/doas /usr/bin/php ' . escapeshellarg($this->config->projectDir . '/cli.php') . ' hosts';
  if ($mode !== '')
    $command .= ' ' . escapeshellarg($mode);
  $output = array();
  $code = 0;
  exec($command . ' 2>&1', $output, $code);
  if ($code !== 0)
    throw new RuntimeException(trim(implode("\n", $output)) ?: 'DHCP apply failed');
  return implode("\n", $output);
}

public function jsonResponse($data, int $status = 200): void {
  throw new \FenPing\Api\ResponseException(new \FenPing\Api\JsonResponse($data, $status));
}

public function jsonError(int $status, string $message): void {
  $this->jsonResponse(array('error' => $message), $status);
}
}
