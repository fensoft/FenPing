...
trait CoreStatusHistoryBehavior
{
public function get_history($ip, $blipSeconds = 120) {
  $stmt = $this->getDb()->prepare("select *, unixepoch(date_begin) as `begin`, unixepoch(date_end) as `end`, unixepoch(date_end)-unixepoch(date_begin) as duration from stats where ip=:ip and date_end > datetime('now', '-7 days') order by id asc");
