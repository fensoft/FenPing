...
trait HttpBehavior
{
public function fenpingHttpRequest(string $url, array $options = array()): array {
...
public function fenpingHttpValidateUrl(string $url): void {
  $parts = parse_url($url);
...
public function fenpingHttpResponseStatus(array $headers): int {
  $status = 0;
...
public function fenpingHttpResponseHeader(array $headers, string $name): ?string {
  $value = null;
...
public function fenpingHttpRedirectUrl(string $baseUrl, string $location): string {
