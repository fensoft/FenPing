...
trait PingBehavior
{
public function pingHosts($ips, $interface, $localIps) {
  $hosts = array();
  ...
  foreach ($ips as $ip)
    $hosts[] = $this->pingHost($ip, $interface, $localIps, $localMac, $arpTable, $icmpUp, null);

  $this->savePingHosts($hosts);
  return $hosts;
}

public function pingHost($ip, $interface, $localIps = array(), $localMac = null, $arpTable = null, $icmpUp = null, $db = null) {
  $mac = "";

  if (in_array($ip, $localIps, true)) {
    $host = array("ip" => $ip, "mac" => $localMac === null ? $this->localMac($interface) : $localMac, "status" => "Up");
    $this->savePingHost($host, $db);
    return $host;
  }
...
public function savePingHost($host, $db) {
  if ($db === null)
    return;
  ...
  $this->updateStatusHistory($db, $ip, $mac === "" ? null : $mac, $status);
}

public function savePingHosts(array $hosts): void {
  if ($hosts === array())
    return;
  $database = $this->db();
  $statements = $this->pingStatements();
...
public function localMac($interface) {
  if ($interface === "")
    return "";
...
public function readArpTable($preferredInterface) {
...
public function pingSweep($ips, $interface, $skipIps) {
  $result = array();
...
public function singlePing($ip, $interface) {
  $result = $this->pingSweep(array($ip), $interface, array());
  return !empty($result[$ip]);
}

public function createIcmpSocket() {
...
public function icmpPacket($id, $sequence) {
...
public function parseIcmpReply($buffer) {
  if (strlen($buffer) < 28)
    return null;
...
public function arpingReplies($ip, $interface) {
  if ($interface === "")
    return false;
...
public function systemPing($ip, $interface) {
  $cmd = "ping -c 1 -W 1 ";
  if ($interface !== "")
