...
trait ScanXmlParserBehavior
{
public function scanParseXml(string $xml, ?array $metadata = null): array {
...
public function scanParseScripts(string $xml): array {
  ...
  foreach ($this->scanTagBlocks($xml, 'script') as $block) {
    ...
    $scripts[] = array(
      ...
      'nodes' => $this->scanParseScriptNodes($block)
    );
  }
  return $scripts;
}

public function scanParseScriptNodes(string $script): array {
  $nodes = array();
  $stack = array();
