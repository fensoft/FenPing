...
trait ScanSnapshotStoreBehavior
{
public function scanEnsureSnapshot(array $job, array $scan): array {
  $resultHash = $this->scanResultHash($scan);
  $contentHash = $this->scanContentHash($scan);
...
public function scanPersistSnapshot(int $snapshotId, array $scan): void {
  $scopeInsert = $this->db()->prepare("INSERT INTO scan_snapshot_scopes (snapshot_id, protocol, port_begin, port_end) VALUES (:snapshot_id, :protocol, :port_begin, :port_end)");
  ...
  $addressInsert = $this->db()->prepare("INSERT INTO scan_snapshot_addresses (snapshot_id, position, address, address_type, vendor) VALUES (:snapshot_id, :position, :address, :address_type, :vendor)");
  ...
  $hostnameInsert = $this->db()->prepare("INSERT INTO scan_snapshot_hostnames (snapshot_id, position, hostname, hostname_type) VALUES (:snapshot_id, :position, :hostname, :hostname_type)");
  ...
  ");
  $cpeInsert = $this->db()->prepare("INSERT INTO scan_snapshot_port_cpes (port_id, position, cpe) VALUES (:port_id, :position, :cpe)");
  foreach ($scan['ports'] ?? array() as $port) {
    ...
    $portId = (int)$this->db()->lastInsertId();
  ...
  $extraInsert = $this->db()->prepare("INSERT INTO scan_snapshot_extra_ports (snapshot_id, position, state, count) VALUES (:snapshot_id, :position, :state, :count)");
  $reasonInsert = $this->db()->prepare("INSERT INTO scan_snapshot_extra_reasons (extra_port_id, position, reason, count, protocol, ports) VALUES (:extra_port_id, :position, :reason, :count, :protocol, :ports)");
  foreach ($scan['extra_ports'] ?? array() as $position => $extra) {
    ...
    $extraId = (int)$this->db()->lastInsertId();
  ...
  $matchInsert = $this->db()->prepare("INSERT INTO scan_snapshot_os_matches (snapshot_id, position, name, accuracy) VALUES (:snapshot_id, :position, :name, :accuracy)");
  $classInsert = $this->db()->prepare("INSERT INTO scan_snapshot_os_classes (os_match_id, position, vendor, os_family, os_generation, device_type, accuracy) VALUES (:os_match_id, :position, :vendor, :os_family, :os_generation, :device_type, :accuracy)");
  $osCpeInsert = $this->db()->prepare("INSERT INTO scan_snapshot_os_cpes (os_class_id, position, cpe) VALUES (:os_class_id, :position, :cpe)");
  ...
  $hopInsert = $this->db()->prepare("INSERT INTO scan_snapshot_trace_hops (snapshot_id, position, protocol, port, ttl, ip, hostname, rtt) VALUES (:snapshot_id, :position, :protocol, :port, :ttl, :ip, :hostname, :rtt)");
...
public function scanPersistScripts(int $snapshotId, ?int $portId, array $scripts): void {
  $scriptInsert = $this->db()->prepare("INSERT INTO scan_snapshot_scripts (snapshot_id, port_id, position, script_id, output) VALUES (:snapshot_id, :port_id, :position, :script_id, :output)");
  $nodeInsert = $this->db()->prepare("INSERT INTO scan_snapshot_script_nodes (script_id, parent_id, position, node_type, node_key, value) VALUES (:script_id, :parent_id, :position, :node_type, :node_key, :value)");
  foreach ($scripts as $position => $script) {
    ...
    $scriptDbId = (int)$this->db()->lastInsertId();
    $nodeIds = array();
    ...
    foreach ($script['nodes'] ?? array() as $index => $node) {
      ...
      $nodeIds[$index] = (int)$this->db()->lastInsertId();
    }
  }
}
...
public function scanReadSnapshot(string $ip, ?array $metadata = null): ?array {
  if ($metadata === null)
    $metadata = $this->scanMetadataBestResult($ip);
  if ($metadata === null || (int)($metadata['snapshot_id'] ?? 0) <= 0)
    return null;

  if (isset($metadata['id'])) {
    $execution = $this->db()->prepare("
  ...
  $exists = $this->db()->prepare("SELECT COUNT(*) FROM scan_snapshots WHERE id=:id AND ip=:ip");
  ...
  $scan = array(
    ...
    'trace' => array(),
    'metadata' => $metadata,
    'xml' => isset($metadata['id']) ? $this->scanXmlUrl($ip, (int)$metadata['id']) : null
  );

  $stmt = $this->db()->prepare("SELECT protocol, port_begin, port_end FROM scan_snapshot_scopes WHERE snapshot_id=:id ORDER BY protocol, port_begin, port_end");
  $stmt->execute(array('id' => $snapshotId));
  while ($row = $stmt->fetch(PDO::FETCH_ASSOC))
    $scan['port_scope'][(string)$row['protocol']][] = array((int)$row['port_begin'], (int)$row['port_end']);

  $stmt = $this->db()->prepare("SELECT address, address_type, vendor FROM scan_snapshot_addresses WHERE snapshot_id=:id ORDER BY position, id");
  $stmt->execute(array('id' => $snapshotId));
  while ($row = $stmt->fetch(PDO::FETCH_ASSOC))
    $scan['addresses'][] = array('addr' => $row['address'], 'type' => $row['address_type'], 'vendor' => $row['vendor'] ?? '');

  $stmt = $this->db()->prepare("SELECT hostname, hostname_type FROM scan_snapshot_hostnames WHERE snapshot_id=:id ORDER BY position, id");
  $stmt->execute(array('id' => $snapshotId));
  while ($row = $stmt->fetch(PDO::FETCH_ASSOC))
    $scan['hostnames'][] = array('name' => $row['hostname'], 'type' => $row['hostname_type']);

  $portsById = array();
  $stmt = $this->db()->prepare("SELECT * FROM scan_snapshot_ports WHERE snapshot_id=:id ORDER BY port, protocol, id");
  $stmt->execute(array('id' => $snapshotId));
  while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
  ...
  $stmt = $this->db()->prepare("SELECT c.port_id, c.cpe FROM scan_snapshot_port_cpes c INNER JOIN scan_snapshot_ports p ON p.id=c.port_id WHERE p.snapshot_id=:id ORDER BY c.port_id, c.position");
  $stmt->execute(array('id' => $snapshotId));
  while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
  ...
  $scriptRows = array();
  $stmt = $this->db()->prepare("SELECT id, port_id, script_id, output FROM scan_snapshot_scripts WHERE snapshot_id=:id ORDER BY port_id IS NOT NULL, port_id, position, id");
  $stmt->execute(array('id' => $snapshotId));
  while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
  ...
  if (count($scriptRows) !== 0) {
    $stmt = $this->db()->prepare("SELECT n.id, n.script_id, n.parent_id, n.node_type, n.node_key, n.value FROM scan_snapshot_script_nodes n INNER JOIN scan_snapshot_scripts s ON s.id=n.script_id WHERE s.snapshot_id=:id ORDER BY n.script_id, n.id");
    $stmt->execute(array('id' => $snapshotId));
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
  ...
  $extraById = array();
  $stmt = $this->db()->prepare("SELECT id, state, count FROM scan_snapshot_extra_ports WHERE snapshot_id=:id ORDER BY position, id");
  $stmt->execute(array('id' => $snapshotId));
  while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
    $extraById[(int)$row['id']] = count($scan['extra_ports']);
    $scan['extra_ports'][] = array('state' => $row['state'], 'count' => (int)$row['count'], 'reasons' => array());
  }
  $stmt = $this->db()->prepare("SELECT r.* FROM scan_snapshot_extra_reasons r INNER JOIN scan_snapshot_extra_ports e ON e.id=r.extra_port_id WHERE e.snapshot_id=:id ORDER BY r.extra_port_id, r.position, r.id");
  $stmt->execute(array('id' => $snapshotId));
  while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
  ...
  $matchesById = array();
  $classesById = array();
  $stmt = $this->db()->prepare("SELECT id, name, accuracy FROM scan_snapshot_os_matches WHERE snapshot_id=:id ORDER BY position, id");
  $stmt->execute(array('id' => $snapshotId));
  while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
    $matchesById[(int)$row['id']] = count($scan['os_matches']);
    $scan['os_matches'][] = array('name' => $row['name'], 'accuracy' => (int)$row['accuracy'], 'classes' => array());
  }
  $stmt = $this->db()->prepare("SELECT c.* FROM scan_snapshot_os_classes c INNER JOIN scan_snapshot_os_matches m ON m.id=c.os_match_id WHERE m.snapshot_id=:id ORDER BY c.os_match_id, c.position, c.id");
  $stmt->execute(array('id' => $snapshotId));
  while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
  ...
  $stmt = $this->db()->prepare("SELECT c.os_class_id, c.cpe FROM scan_snapshot_os_cpes c INNER JOIN scan_snapshot_os_classes oc ON oc.id=c.os_class_id INNER JOIN scan_snapshot_os_matches m ON m.id=oc.os_match_id WHERE m.snapshot_id=:id ORDER BY c.os_class_id, c.position");
  $stmt->execute(array('id' => $snapshotId));
  while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
  ...
  $scan['os'] = $this->scanSelectOsMatches($scan['os_matches']);

  $stmt = $this->db()->prepare("SELECT protocol, port, ttl, ip, hostname, rtt FROM scan_snapshot_trace_hops WHERE snapshot_id=:id ORDER BY position, id");
  $stmt->execute(array('id' => $snapshotId));
  while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
    $scan['trace'][] = array(
