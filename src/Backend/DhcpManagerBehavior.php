...
public function validateGeneratedDnsmasqFiles(array $files): void {
  ...
  foreach ($this->generatedDnsmasqLines($files['dhcpHosts']) as $line) {
    ...
    $nameAndLeaseValid = ($count === 4 && $parts[3] === 'infinite')
      || ($count === 5 && $this->normalizeDhcpHostname($parts[3], true) === $parts[3] && $parts[4] === 'infinite');
    if (($count !== 4 && $count !== 5) || $this->normalizeDhcpMac($parts[0], true) !== $parts[0]
      || preg_match('/^set:[A-Za-z0-9-]+$/', $parts[1]) !== 1
      || $this->normalizeDhcpIp($parts[2], true) !== $parts[2]
      || !$nameAndLeaseValid)
  ...
  foreach ($this->generatedDnsmasqLines($files['dnsHosts']) as $line) {
    ...
    if ($parts === false || count($parts) < 2 || $this->normalizeDhcpIp(array_shift($parts), true) === null)
