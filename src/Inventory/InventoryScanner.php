...
public function inventoryScan(string $ip, string $mode = 'deep', ?int $scanId = null): array {
  ...
  if ($scanId === null)
    $scanId = $this->jobs->start($ip, $mode);
  $tmp = tempnam(sys_get_temp_dir(), 'fenping-nmap-');
  if ($tmp === false) {
    $this->jobs->fail($scanId, 'failed to create temporary nmap file');
  ...
  try {
    ...
    $this->inventoryRunScanProcess($command, $this->profiles->timeout($mode), $scanId, $mode);
    $xml = file_get_contents($tmp);
    if ($xml === false)
      throw new RuntimeException("failed to read nmap result for $ip");

    $xml = $this->codec->normalize($xml);
    $scan = $this->codec->parse($xml, array('ip' => $ip));
    $status = $scan['status'] ?: 'unknown';
    $saved = $status === 'up';
    $changed = $this->jobs->complete($scanId, $scan);
    if ($saved)
      $this->notifications->notifyPortChangesForScan($scanId);
    $this->retention->prune($ip);
    return array(
      'saved' => $saved,
      'changed' => $changed,
      'status' => $status
    ...
    $this->jobs->markCancelled($scanId);
    $this->retention->prune($ip);
    throw $e;
  } catch (InventoryTimeoutException $e) {
    $this->jobs->timeout($scanId, $e->getMessage());
    $this->retention->prune($ip);
    throw $e;
  } catch (Throwable $e) {
    $this->jobs->fail($scanId, $e->getMessage());
    $this->retention->prune($ip);
    throw $e;
  } finally {
    @unlink($tmp);
...
public function inventoryRunScanProcess(array $command, int $timeoutSeconds, int $scanId, string $profile): void {
  if ($this->jobs->cancellationRequested($scanId))
    ...
    while (true) {
      foreach (array(1, 2) as $descriptor) {
        ...
        while (($newline = strpos($buffers[$descriptor], "\n")) !== false) {
          ...
          $this->jobs->updateProgress($scanId, $parsed['phase'], $progress);
        }
      }
  ...
  if ($terminationReason === 'cancelled' || $this->jobs->cancellationRequested($scanId))
    throw new InventoryCancelledException('scan cancelled by operator');
  ...
  $this->jobs->updateProgress($scanId, 'finalizing', 99);
}
