...
public function statsMaxId(): int {
  $value = $this->database->connection()->query("SELECT COALESCE(MAX(id), 0) FROM stats")->fetchColumn();
  return (int)$value;
}
...
public function discordIpConflictPayloads(array $changes): array {
  ...
  foreach (array_chunk($changes, 10) as $chunk) {
    ...
    $payloads[] = $this->payloads->discordApplyMention(array(
      'username' => 'FenPing',
      'embeds' => $embeds
    ));
  }
...
public function discordIpConflictEmbed(array $change): array {
  ...
  $embed = array(
    ...
    'fields' => array(
      $this->payloads->discordEmbedField('IP', $ip, true),
      $this->payloads->discordEmbedField('Network', $network, true),
      $this->payloads->discordEmbedField('Devices', implode("\n", $deviceLines), false),
      $this->payloads->discordEmbedField('Time', $occurredAt, false)
    )
  );
  if ($timestamp !== false)
...
public function sendDiscordRestartNotification(): bool {
  if (!$this->discordNotificationsEnabled() || !$this->rules->notificationRules()['restart'])
    return false;
...
public function discordRestartPayload(): array {
  ...
  $fields = array(
    $this->payloads->discordEmbedField('Host', $host, true),
    $this->payloads->discordEmbedField('Time', date('Y-m-d H:i:s T'), true)
  );

  if (($this->config->network ?? '') !== '')
    $fields[] = $this->payloads->discordEmbedField('Network', $this->config->network . '.0/24', true);
  if (($this->config->applianceIp ?? '') !== '')
    $fields[] = $this->payloads->discordEmbedField('Address', $this->config->applianceIp, true);

  return $this->payloads->discordApplyMention(array(
    'username' => 'FenPing',
    'embeds' => array(array(
...
public function discordNotificationPayloads(array $changes): array {
  ...
  foreach (array(false, true) as $important) {
    ...
    foreach (array_chunk($group, 10) as $chunk) {
      $embeds = array();
      foreach ($chunk as $change)
        $embeds[] = $this->payloads->discordChangeEmbed($change);
      $payloads[] = $this->payloads->discordApplyMention(array(
        'username' => 'FenPing',
        'embeds' => $embeds
      ));
...
public function discordPortNotificationPayloads(array $changes): array {
  ...
  foreach (array(false, true) as $important) {
    ...
    foreach (array_chunk($group, 10) as $chunk) {
      $embeds = array();
      foreach ($chunk as $change)
        $embeds[] = $this->payloads->discordPortChangeEmbed($change);
      $payloads[] = $this->payloads->discordApplyMention(array(
        'username' => 'FenPing',
        'embeds' => $embeds
      ));
...
public function discordPostPayload(array $payload): bool {
  $payload = $this->payloads->discordApplyMention($payload);
