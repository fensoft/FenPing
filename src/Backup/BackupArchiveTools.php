...
final readonly class BackupArchiveTools
{
    public function __construct(private ConfigManager $dhcp)
    {
    }
...
public function backupValidateDocument(array $document, string $format, string $label): void {
  ...
  if ($match[1] !== '1' || version_compare($version, BackupDatabaseDocument::BACKUP_VERSION, '<'))
