...
final readonly class BackupDatabaseDocument
{
...
public function backupRestoreDatabase(array $document): void {
  ...
  $orderedTables = $this->tables->backupTableNames();
  try {
    $this->database->beginImmediate();
    foreach (array_reverse($orderedTables) as $table) {
      if (isset($schema[$table]))
        $database->exec('DELETE FROM ' . $this->tools->backupQuoteIdentifier($table));
    }
    ...
    foreach ($orderedTables as $table) {
      ...
      $sql = 'INSERT INTO ' . $this->tools->backupQuoteIdentifier($table)
        . ' (' . implode(', ', array_map($this->tools->backupQuoteIdentifier(...), $columns)) . ')'
...
public function backupCurrentSchema(): array {
  ...
  foreach ($this->tables->backupTableNames() as $table) {
    $exists = $this->database->connection()->prepare("SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name=:name");
    $exists->execute(array('name' => $table));
    if ((int)$exists->fetchColumn() !== 1)
      continue;
    $stmt = $this->database->connection()->query('PRAGMA table_xinfo(' . $this->tools->backupQuoteIdentifier($table) . ')');
...
public function backupNetbootIndex(): array {
  ...
  $rows = array();
  while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
    $path = $this->filesystem->backupNetbootDir() . '/' . basename((string)$row['filename']);
    $row['id'] = (int)$row['id'];
...
public function backupManifest(string $database, array $databaseCounts): array {
  ...
  return array(
    ...
    'counts' => array(
      'netboot_files' => $this->filesystem->backupCountFiles($this->filesystem->backupNetbootDir()),
