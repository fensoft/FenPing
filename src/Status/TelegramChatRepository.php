...
final readonly class TelegramChatRepository
{
...
public function telegramEnsureBotState(): void {
  ...
  $current = $this->database->connection()->query("
...
public function telegramChatSelectionUpdate(mixed $chatId): ?string {
  ...
  if ($target !== null) {
    $exists = $this->database->connection()->prepare('SELECT COUNT(*) FROM telegram_known_chats WHERE chat_id=:chat_id');
...
public function telegramRefreshKnownChats(): array {
  ...
  for ($page = 0; $page < 100; $page++) {
    ...
    $updates = $this->api->telegramApiRequest('getUpdates', $payload);
...
public function telegramOptionalText(mixed $value): ?string {
  ...
  return $value === '' ? null : $this->textValue($value, 200);
}
