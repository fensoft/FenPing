...
The other explicit subcommands are `restart`, `start`, `destroy`, `restore`, `dev`, `demo`, and `rollback`; invoking `fenping.sh` without a subcommand remains an alias for `restart`. `restore` accepts one archive under `data/backups`, development mode accepts either `--no-backup` or `restore <backup.tgz>`, and `publish` accepts at most one optional version. `start` uses only the configured image already available locally and does not create an upgrade checkpoint. `destroy` removes the Compose container without deleting bind-mounted data or local images. Restore validates the host-side archive before removing the Compose container and runs through a fresh one-off container; `dev restore` first builds the local `dev` image so recovery does not depend on a published image tag.
