...
function fenpingHttpRequest(string $url, array $options = array()): array {
  ...
  for ($redirects = 0; ; $redirects++) {
    fenpingHttpValidateUrl($currentUrl);
    $response = fenpingHttpRequestOnce($currentUrl, $method, $headers, $body, $timeout, $maxBytes);
    $location = fenpingHttpResponseHeader($response['headers'], 'location');
    ...
    $currentUrl = fenpingHttpRedirectUrl($currentUrl, $location);
...
function fenpingHttpRequestOnce(string $url, string $method, array $headers, ?string $body, float $timeout, int $maxBytes): array {
  $headerLines = array();
  ...
  try {
    ...
    $status = fenpingHttpResponseStatus($responseHeaders);
    if ($status === 0)
...
function fenpingHttpValidateUrl(string $url): void {
  $parts = parse_url($url);
...
function fenpingHttpResponseStatus(array $headers): int {
  $status = 0;
...
function fenpingHttpResponseHeader(array $headers, string $name): ?string {
  $value = null;
...
function fenpingHttpRedirectUrl(string $baseUrl, string $location): string {
