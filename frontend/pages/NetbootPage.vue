<template>
  <section>
    ...
    <div class="netboot-header">
      <div><h2>{{ t('Netboot images') }}</h2><div class="text-secondary small">{{ t('Private boot files served by dnsmasq TFTP') }}</div></div>
    ...
    <div v-if="!isAuthenticated" class="alert alert-info" role="alert">{{ t('Guest mode is read only. You can browse and download images.') }}</div>
    <form v-if="isAuthenticated" class="netboot-upload" @submit.prevent="upload">
      <label class="form-label netboot-name-field">{{ t('Name') }}<input v-model.trim="uploadForm.name" class="form-control form-control-sm" type="text" :placeholder="t('Optional display name')" /></label>
      <label class="form-label netboot-file-field">{{ t('File') }}<input ref="fileInput" class="form-control form-control-sm" type="file" accept=".efi,.kpxe,.kkpxe,.kkkpxe,.pxe,.lkrn,.0,.ipxe" @change="onFile" /></label>
      <button class="btn btn-primary btn-sm" type="submit" :disabled="uploading"><AppIcon name="upload" class="me-1" :class="{ 'is-spinning': uploading }" />{{ t('Upload') }}</button>
    </form>
    <div class="table-wrap">
      <table class="table table-sm netboot-table">
        ...
        <thead><tr><th>{{ t('Name') }}</th><th>{{ t('File') }}</th><th class="text-nowrap">{{ t('Size') }}</th><th class="text-center text-nowrap">{{ t('Hosts') }}</th><th>{{ t('Created') }}</th><th v-if="isAuthenticated" class="text-end">{{ t('Actions') }}</th></tr></thead>
        <tbody>
          <tr v-if="loading && images.length === 0"><td class="text-secondary text-center py-4" :colspan="isAuthenticated ? 6 : 5">{{ t('Loading') }}</td></tr>
          <tr v-else-if="!loading && images.length === 0"><td class="text-secondary text-center py-4" :colspan="isAuthenticated ? 6 : 5">{{ t('No images') }}</td></tr>
          <tr v-for="image in images" :key="image.id">
            ...
            <td v-if="isAuthenticated" class="text-end"><button class="btn btn-outline-danger btn-sm icon-btn" type="button" :title="t('Delete image')" :disabled="loading" @click="remove(image)"><AppIcon name="trash" /></button></td>
          </tr>
        </tbody>
      </table>
    </div>
...
async function upload() {
  ...
  if (!uploadForm.value.file) { error.value = t('Choose a file to upload'); return; }
...
async function remove(image) {
  if (!props.isAuthenticated) return emit('login');
  if (!window.confirm(t('Delete {name}?', { name: image?.name || image?.filename || t('this image') }))) return;
  const signal = mutationRequest.nextSignal();
  loading.value = true;
  error.value = '';
  try {
